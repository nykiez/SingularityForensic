...
namespace CDFC.Parse.Android.DeviceObjects {
    //未知分区，尚未细分;
    public class AndroidUnknownParititon : Partition {
        /// <summary>
        /// 其他分区的构造方法;
        /// </summary>
        /// <param name="parent">父文件为设备</param>
        ///<param name="partTabInfo">分区结构（自定义）</param>
        public AndroidUnknownParititon(TabPartInfo partTabInfo, Device parent) : base(parent) {
            ...
            if (TabPartInfo != null && TabPartInfo.StPartInfo != null) {
                ...
                if (device != null) {
                    ...
                    if (TabPartInfo.FSType != FileSystemType.EXT4) {
                        ...
                        Logger.WriteLine($"{nameof(AndroidUnknownParititon)}->{nameof(LoadContent)}:Wrong FsType-{TabPartInfo.FSType}");
                        return;
                    }
                    #endregion
                }
                else {
                    Logger.WriteLine($"{nameof(AndroidUnknownParititon)}->{nameof(LoadContent)}:Failed to convert to Device.");
                }
            }
            else {
                Logger.WriteLine($"{nameof(AndroidUnknownParititon)}->{nameof(LoadContent)}:value can't be null-{nameof(TabPartInfo)}");
            }
        }
