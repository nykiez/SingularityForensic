...
namespace CDFC.Info.Android {
    ...
    public class ContactDbModel:EncryptAndDeleteInfoDbModel {
        ...
        [NotMapped]
        internal List<PhoneNumber> numbers { get; set; }
        public IEnumerable<PhoneNumber> Numbers => numbers?.Select(p => p);
        public int PhoneNumbersCount => numbers?.Count() ?? 0;

        [NotMapped]
        internal List<EmailAddress> emails { get; set; }
        public IEnumerable<EmailAddress> Emails => emails?.Select(p => p);
