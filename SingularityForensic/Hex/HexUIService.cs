...
namespace SingularityForensic.Hex {
    [Export]
    public partial class HexUIService : ExtensibleBindableBase{
    ...
    public partial class HexUIService {
        private void RaiseCanExcute() {
        ...
        private void FindHex(FindHexValueSetting setting) {
            ...
            if (!setting.IsBlockSearch) {
                ServiceProvider.Current.GetInstance<IHexService>()?.FindNextBytes(curHexContext, setting.HexBytes);
            }
        ...
        private void FindText(FindTextStringSetting setting) {
            ...
            if (!setting.IsBlockSearch) {
                ServiceProvider.Current.GetInstance<IHexService>()?.
                    FindNextString(curHexContext, setting.FindText);
            }
            else if (setting.BlockSize != null && setting.BlockOffset != null) {
                ServiceProvider.Current.GetInstance<IHexService>()?.
                    FindNextString(curHexContext, setting.FindText, true,
                    setting.BlockSize.Value, setting.BlockOffset.Value);
            }
        ...
        //private static MenuButtonItem goToOffsetMenuItem2;
        //[Export]
        //public static MenuButtonItem GoToOffsetMenuItem2
        //    => goToOffsetMenuItem2 ?? (goToOffsetMenuItem2 = new MenuButtonItem(
        //    MenuConstants.MenuMainGroup,
        //    ServiceProvider.Current?.GetInstance<ILanguageService>()?.FindResourceString("GoToOffset")) {
        //        Command = new DelegateCommand(() => {
        //            MsgBoxService.Show("Fuck you");
        //        }),
