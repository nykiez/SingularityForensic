                    ...
                    ThreadPool.QueueUserWorkItem(cb => {
                        ...
                        slUnits.ForEach(p => {
                            if (p is AdbInfoTreeUnit<IInfo>) {
                                ...
                                try {
                                    ...
                                    var abPath = $"{SingularityCase.Current.Path}/{Device.Serial}/{dtString}/backup.ab";
                                    ...
                                    if (bRes) {
                                        ...
                                        var bParser = BackupParser.Create(abPath, 
                                            $"{SingularityCase.Current.Path}/{Device.Serial}/{dtString}/backup",
                                            err => {
                        ...
                        slUnits.ForEach(p => {
                            if (p is AdbInfoTreeUnit<IInfo>) {
                                ...
                                var node = p as AdbBackUpFilesTreeUnit;
                                if (node.Succeed) {
                                    var container = new BackUpFilesContainer($"{SingularityCase.Current.Path}/", node.Direct);
                            ...
                            msg.DoWork += (sender, e) => {
                                ...
                                containers.ForEach(con => {
                                    ...
                                    MInfoTypeHelper.GetMInfoTypeBox(con.InfoType) == MInfoTypeBox.AdbFile) {
                                        try {
                                            if (Directory.Exists(SingularityCase.Current?.Path)) {
                                                foreach(var f in mulCon.Infoes){
                                                    ...
                                                    DownloadFileFromUrl(urlInfo.Url, $"{SingularityCase.Current.Path}/AdbDevices/{Device.Serial}",
                                                        per => {
                                                            if (totalSize == 0) {
                                                ...
                                                EventLogger.Logger.WriteLine($"{nameof(AdbInfoesCheckedViewModel)}->{nameof(ProgressMessageBox)}({nameof(msg)}):CasePath{SingularityCase.Current?.Path} doesn't exist.");
                                            }
