using SingularityForensic.Android.Info.Resouces;
        ...
        public static string GetClassLabel(string afc) {
            switch (afc) {
                ...
                case ForensicClassBrowser:
                    return "浏览器";
                case AndQQ:
                    return  ServiceProvider.Current?.GetInstance<ILanguageService>()?.FindResourceString("AndQQ");
                case AndWeChat:
                    return  ServiceProvider.Current?.GetInstance<ILanguageService>()?.FindResourceString("AndWeChat");
            }
            return string.Empty;
        }
